// crates/model/src/thevie/synapse.rs
// =====================================================
// Synapse v2.1 — Connexion Neurone à Neurone
// Version améliorée et plus robuste
// =====================================================

use serde::{Serialize, Deserialize};
use std::time::{SystemTime, UNIX_EPOCH};

pub type NeuronId = u64;

#[derive(Debug, Clone, Serialize, Deserialize, PartialEq)]
pub struct Synapse {
    pub from: NeuronId,
    pub to: NeuronId,
    pub strength: f32,
    pub usage_count: u32,
    pub last_used: u64,
    pub decay_rate: f32,
    pub created_at: u64,
}

impl Synapse {
    pub fn new(from: NeuronId, to: NeuronId) -> Self {
        let now = Self::now_millis();
        Self {
            from,
            to,
            strength: 0.5,
            usage_count: 0,
            last_used: now,
            decay_rate: 0.008,
            created_at: now,
        }
    }

    /// Renforce la connexion (Hebbian)
    pub fn strengthen(&mut self, amount: f32) {
        self.strength = (self.strength + amount).min(1.0);
        self.usage_count += 1;
        self.last_used = Self::now_millis();
    }

    /// Affaiblit la connexion (Anti-Hebbian)
    pub fn weaken(&mut self, amount: f32) {
        self.strength = (self.strength - amount).max(0.08);
        self.last_used = Self::now_millis();
    }

    /// Dégradation naturelle
    pub fn decay(&mut self) {
        self.strength = (self.strength - self.decay_rate).max(0.08);
    }

    /// Vérifie si la synapse est active
    pub fn is_active(&self) -> bool {
        self.strength > 0.12 && self.usage_count > 0
    }

    /// Âge de la synapse en secondes
    pub fn age_seconds(&self) -> u64 {
        (Self::now_millis() - self.created_at) / 1000
    }

    fn now_millis() -> u64 {
        SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .unwrap()
            .as_millis() as u64
    }
}

impl Default for Synapse {
    fn default() -> Self {
        Self::new(0, 0)
    }
}